...
class SearchMoviesCubit extends Cubit<SearchMovieState> {
  ...
  void onSubmitted(String value) async {
    emit(state.copyWith(status: MovieStatus.loading, error: '', movies: []));
    try {
      List movies = [];
      int lessthen;
      ...
      if (movies.isEmpty) {
        emit(state.copyWith(
          status: MovieStatus.notFound, error: '', movies: [],
        ));
      } else {
        emit(state.copyWith(status: MovieStatus.success, movies: movies, error: ''));
      }
    } catch (e) {
      print(e.toString());
      emit(state.copyWith(status: MovieStatus.error, error: '', movies: []));
    }
  }
}
